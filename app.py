...
# Mapping of filenames to product names
file_mapping = {
    "BeltMetrics_Training_Quiz.xlsx": "BeltMetrics",
    "General_Product_Training.xlsx": "General",
    "PortaMetrics_Training_Quiz.xlsx": "PortaMetrics",
    "LoaderMetrics_Features_Monitoring.xlsx": "LoaderMetrics",
    "TruckMetrics_Training_Quiz.xlsx": "TruckMetrics",
    "LoaderMetrics_Training_Quiz.xlsx": "LoaderMetrics",
    "ShovelMetrics_Training_Quiz.xlsx": "ShovelMetrics",
    "ShovelMetrics_Training_Overview.xlsx": "ShovelMetrics",
    "ShovelMetrics_GET_Monitoring.xlsx": "ShovelMetrics",
    "ShovelMetrics_Rock_Monitoring.xlsx": "ShovelMetrics",
}

# Function to extract product name from filename using mapping
def extract_product_name(filename):
    return file_mapping.get(filename, "Unknown Product")  # Use "Unknown Product" if filename is not in mapping
...
if not EXCEL_FILE_URLS:
    ...
    for url in EXCEL_FILE_URLS:
        try:
            ...
            title = url.split("/")[-1]  # Extract the filename from the URL
            product_name = extract_product_name(title)  # Extract product name using the mapping
